...
router.post('/searchWithTime', async (req, res) => {
    const { startDate, endDate, processingLevel } = req.body;
    console.log(startDate, endDate, processingLevel);
    ...
    try {
        ...
        if (data.Items) {
            ...
            const filteredTimes = item.time.filter((timeEntry: any) => {
                const timeKey = Object.keys(timeEntry)[0];
                const timeDate = new Date(`${item.date} ${timeKey.slice(0, 2)}:${timeKey.slice(2)}`);
                return timeDate >= start && timeDate <= end;
            });
            return { ...item, time: filteredTimes };
            }).filter(item => {
